...
HttpServer::HttpServer(const int port, RelayController &controller) : port_(port), controller_(controller) {
...
void HttpServer::run() const {
    while (true) {
        if (const int client = accept(server_socket_, nullptr, nullptr); client >= 0) {
...
void HttpServer::send_response(const int client, const int status, const std::string &content_type, const std::string &body) {
...
void HttpServer::handle_client(const int client) const {
    char request[4096]{};
    ...
    try {
        ...
        if (const std::string prefix = "/api/relays/"; method == "POST" && path.rfind(prefix, 0) == 0) {
