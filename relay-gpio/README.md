...
`libgpiod-dev` and `libgpiodcxx-dev` when available. For Fedora, install `libgpiod-devel` and `libgpiod-c++`. This project uses the libgpiod 2.x C++ bindings (`gpiod.hpp`).
