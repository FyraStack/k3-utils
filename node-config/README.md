...
for `IMAGE`, and network access to GHCR plus the Kairos AuroraBoot image. Set
`PUSH_IMAGE=0` only if the image is already available from the configured
registry; a local-only image cannot be consumed by the AuroraBoot container.
